...
📢 **次要通知**：此部分主要用于存放“相对不太重要”但又需要“长期保留”的通知，以减少其对重要内容的干扰。
 ...
 - 🗓️ 2026.07.06 🗓️ 昨日，`Xray`内核的开发团队宣布停止维护中国翻墙生态，只针对俄罗斯和伊朗进行维护，[详见此](https://t.me/s/projectXtls/3373)（注：该作者发言相当情绪化，可能引起不适，慎入）。个人观点（注：以下所说的停止维护指的是停止针对中国用户翻墙需求的维护，相关内核和协议依然是会更新的）：① 受影响的主要是使用了 `Xray` 内核的代理软件以及使用了 `Vless`，`XHTTP`，`Reality` 协议的机场（这几个协议和 `Xray` 内核均出自同一开发团队）。② `Xray` 的停止维护对大部分用户无影响，因为使用 `Xray` 的用户数量是远不及 `Mihomo` 的（当然，这并不代表 `Xray` 不好，只不过 `Mihomo` 的 GUI 普遍颜值高，更容易受用户青睐）。即使是使用 `Xray` 的客户端，短期内也照样能用。就如当初的 CFW 删库，依然能用很久。哪怕是现在，你硬要用也可以，只不过存在兼容性差以及安全性低等缺点。③ `Vless` 的停止维护对大部分用户更是没有影响，因为节点用什么协议是机场主需要考虑的事情，用户只需保证自己所使用的代理软件能够支持机场所使用的协议即可。实际上，自 4 月份的拔线事件后，市面上使用 `Vless` 的机场已经少很多了，大部分都去使用 `AnyTLS` 或封端了。不过，这并不能说明 `AnyTLS` 就比 `Vless` 更好。④ 至于 `XHTTP`，目前使用该协议的机场极其少，它的停止维护对用户的影响就更是微乎其微了。⑤ `Reality` 一般会搭配 `Vless` 或 `XHTTP` 进行使用，相关影响参考这两个协议即可。⑥ `Xray` 的开发团队与其他开发者以及用户的爱恨情仇，感兴趣的自行了解，本人无资格做任何评价。
...
👇 **机场购前须知**：
 ...
 - 🔒：**表示该机场只能使用其提供的客户端**（客户端的下载方式通常在官网的“使用文档”中有说明），**不能使用订阅链接导入到代理工具**，就类似于传统的 `VPN` 软件。这种方式的好处是能够较有效防止通报与攻击，不过也有不少坏处，如：部分设备可能不支持（如软路由，iOS，Linux。不同机场支持情况不同，具体支持情况请前往官网查看。购买这类机场前务必查看该机场是否支持自己的设备），客户端安全性未知（如果担心客户端会泄露个人隐私或是携带病毒等，那么就不要选择带有该标识的机场），可玩性较低（第三方代理工具一般能折腾的地方较多，而私有客户端往往功能较单一），切换机场麻烦（对于有多家机场的人来说，想在多家机场的节点中来回切换，原先只需要在一个软件中操作，而这种情况则需要在多个软件中操作）。另外，这类机场由于强制使用客户端，因此，其无论使用的协议多新多罕见，我都不提供标注（做标注的目的是防止大家使用的代理工具不支持相应协议），入口信息也不提供（准确说是我几乎无法获取，这也正是其防通报与攻击的原因。不过，通过抓包的方式其实还是可以获取，但是会麻烦很多）。总而言之，这种方式是一把双刃剑，如果无法接受其诸多缺点，那么请直接避开带有这类标识的机场。但即使能接受，**购买套餐前**，**建议先下载相应的客户端**，**并确保能成功登录**，**再购买**，防止因各种原因无法使用客户端而“白买”。另外，**如果发现所有节点都无法使用且长时间未恢复**，**那可能是当前版本的客户端因存在某些 BUG 而被“弃用”了**，**此时建议去机场官网下载新版本的客户端**（**客户端本身不一定支持更新功能**）。使用机场客户端时，个人建议将软件中所有 `IPv6` 相关的设置统统关闭，确实有需求才开启，这样能减少一些奇奇怪怪的问题。
 ...
 - 类型：描述了机场的主要线路类型是直连还是中转（带 `+` 的直连是经过特殊优化的直连机场，价格与体验基本无异于中转机场，但是防失联能力也跟中转机场一样较弱，两者都有可能出现节点全部不可用的情况。而普通直连机场价格虽然最低，但是在防失联能力反而最强，极少出现节点全部不可用的情况）（`2026.05.31` 补充：自 2026 年 4 月前后的拔线事件后，市面上的中转机场基本都不再用国内机器作为入口（准确说是想用也用不了。因为现在很多国内机房都部署了一款名为“傲盾”的软件，专门针对国内中转机场。这种情况下还用国内中转机器，对于机场主来说就是一种高风险，高成本的行为），改为使用国外机器作为入口，即所谓的海外中转，因此，以下机场的“中转”现在默认都为“海外中转”，即使有部分“漏网之鱼”还能用国内入口，恐怕也用不了多久，除非政策放宽。海外中转的延迟与稳定性从理论上来说自然是比不上国内中转，但也是目前相对较好的方案了，不会因通报造成大量甚至所有节点掉线。不过，其防失联能力依旧是不如直连的）。
    ...
    - 注意，**新疆地区具有特殊性**，**该地区大概率无法使用中转机场**，**只建议购买直连机场或直连+机场**。**河南**，**福建**，**江苏这几个地区也比较特殊**，**优先考虑中转机场**（`2026.07.23` 补充：**由于现在市面上绝大部分中转机场都已没有“国内入口”**，**所以对于江苏与福建这两个地区**，**即使选择中转机场**，**大概率也难以获得较好体验**，**尤其是福建**）。如果身处非大陆地区（如港澳台），想通过使用机场来更改 IP，那么只建议直连或直连+机场，因为大部分中转机场不支持非大陆地区使用。 
...
| 2020.04.22 | [次元雲](https://ciyy.one/#/register?code=tsL8Me6h)<br>`A` | 中转 | 1 | 17.00 / 158 GB | 125.00 / 520 GB | Null | Null |
...
- M78星云：
     - 目前订阅链接依然能导入少量节点，但大概率完全不可用或体验较差。若想购买该机场，只推荐使用其客户端。
...
| Linux | `Clash for Windows`，`Clash Verge`，`Clash Verge Rev`，`Clash Nyanpasu`，`FlClash`，`V2ranN`，`Nekoray`，`sing-box`，`Hiddify`，`Qv2ray`，`Dae`，`Daed`，`Clash Party`，`GUI For SingBox`，`Sparkle`，`AnyPortal`，`Pantheon`，`Bettbox` |
